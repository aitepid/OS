...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 213（hmac + pbkdf2 + uuid）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 268 | 69 根目录 + 199 内核模块 |
| H-L 总行数 | ~72,000 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 199 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 415 | shell.hl（+hmac/pbkdf2/uuid）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 268 HL / 199 模块 / 0 stub |
