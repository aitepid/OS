...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 312（burrows_wheeler + lyndon + z_function）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 367 | 72 根目录 + 298 内核模块 |
| H-L 总行数 | ~108,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 298 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 912 | shell.hl（+bwt/lyndon/zfunction）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| 验证项 | 结果 |
|---|---|
| `hl-bootstrap build` | ✅ 298 模块编译 + 镜像重建 |
| `validate-workspace` | ✅ 367 HL / 298 模块 / 0 stub |
