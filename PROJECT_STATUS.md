...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 318（io_uring + persistent_seg + rollback_dsu）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 373 | 73 根目录 + 304 内核模块 |
| H-L 总行数 | ~110,400 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 304 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 949 | shell.hl（+io_uring/persistent_seg/rollback_dsu）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| 验证项 | 结果 |
|---|---|
| `hl-bootstrap build` | ✅ 304 模块编译 + 镜像重建 |
| `validate-workspace` | ✅ 373 HL / 304 模块 / 0 stub |
