...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 156（json + websocket + ui_paint）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 211 | 69 根目录 + 142 内核模块 |
| H-L 总行数 | ~54,800 | 根 10,900+ + 内核 ~43,900 |
| `bare-kernel/hl/` 内核模块 | 142 | 编译进 `kernel.bin` |
| `kernel_entry.hl` 行数 | 9,428 | 内核入口 + 命令分发 |
| 编译产出函数数 | 1,950+ | 编译管线实测 |
| 链接符号数 | 2,200+ | linker 实测 |
| `hl-bootstrap.hl` 行数 | 4,306 | 自举编译器（208 函数） |
| `stdlib.hl` 行数 | 1,385 | 标准库（143 函数） |
| `kinterp.hl` 行数 | ~1,280 | 内核解释器 |
| `scripts/*.ps1` | 28 | 构建/验证/诊断 |
| PS1 总行数 | 9,729 | |
| Shell 命令数 | 160 | shell.hl（+json/ws/paint） |

### 门禁结果

| 验证项 | 结果 |
|---|---|
| `validate-workspace` | ✅ 211 HL / 142 模块 / 0 stub |
