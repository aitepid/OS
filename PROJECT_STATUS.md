...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 273（fenwick_tree + segment_tree + cuckoo_filter）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 328 | 69 根目录 + 259 内核模块 |
| H-L 总行数 | ~106,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 259 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 715 | shell.hl（+fenwick/segtree/cuckoo）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 328 HL / 259 模块 / 0 stub |
