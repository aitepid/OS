...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 198（cpio + quoted_printable + uuencode）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 253 | 69 根目录 + 184 内核模块 |
| H-L 总行数 | ~69,100 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 184 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 357 | shell.hl（+cpio/qp/uu）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 253 HL / 184 模块 / 0 stub |
