...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 210（crc + md5 + sha1）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 265 | 69 根目录 + 196 内核模块 |
| H-L 总行数 | ~71,600 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 196 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 405 | shell.hl（+crc/md5/sha1）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 265 HL / 196 模块 / 0 stub |
