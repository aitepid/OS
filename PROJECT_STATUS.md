...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 180（traceroute + semaphore + ui_notepad）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 235 | 69 根目录 + 166 内核模块 |
| H-L 总行数 | ~70,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 166 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 253 | shell.hl（+traceroute/sem/notepad）|
| `test_*.hl` / `test-*.hl` | 19 | |
