...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 279（aho_corasick + kmp + interval_tree）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 334 | 69 根目录 + 265 内核模块 |
| H-L 总行数 | ~112,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 265 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 745 | shell.hl（+ahocorasick/kmp/intervaltree）|
| `test_*.hl` / `test-*.hl` | 19 | |
