...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 171（sqlite + diff + ui_clock）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 226 | 69 根目录 + 157 内核模块 |
| H-L 总行数 | ~67,000 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 157 | 编译进 `kernel.bin` |
| `kernel_entry.hl` 行数 | 9,428 | 内核入口 + 命令分发 |
| 编译产出函数数 | 2,080+ | 编译管线实测 |
| 链接符号数 | 2,350+ | linker 实测 |
| `hl-bootstrap.hl` 行数 | 4,306 | 自举编译器（208 函数） |
| `stdlib.hl` 行数 | 1,385 | 标准库（143 函数） |
| Shell 命令数 | 223 | shell.hl（+sqlite/diff/clock）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 226 HL / 157 模块 / 0 stub |
