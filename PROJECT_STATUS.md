...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 225（bcrypt + rtsp + stun）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 280 | 69 根目录 + 211 内核模块 |
| H-L 总行数 | ~74,800 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 211 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 475 | shell.hl（+bcrypt/rtsp/stun）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 280 HL / 211 模块 / 0 stub |
