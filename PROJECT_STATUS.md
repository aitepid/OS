...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 300（two_sat + hungarian + hopcroft_karp）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 355 | 69 根目录 + 286 内核模块 |
| H-L 总行数 | ~104,700 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 286 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 850 | shell.hl（+2sat/hungarian/hopcroftcarp）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| 验证项 | 结果 |
|---|---|
| `hl-bootstrap build` | ✅ 286 模块编译 + 镜像重建 |
| `validate-workspace` | ✅ 355 HL / 286 模块 / 0 stub |
