...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 177（csv + ping + ui_taskman）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 232 | 69 根目录 + 163 内核模块 |
| H-L 总行数 | ~69,000 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 163 | 编译进 `kernel.bin` |
| `kernel_entry.hl` 行数 | 9,428 | 内核入口 + 命令分发 |
| 编译产出函数数 | 2,160+ | 编译管线实测 |
| 链接符号数 | 2,450+ | linker 实测 |
| `hl-bootstrap.hl` 行数 | 4,306 | 自举编译器（208 函数） |
| `stdlib.hl` 行数 | 1,385 | 标准库（143 函数） |
| Shell 命令数 | 245 | shell.hl（+csv/ping/taskman）|
| `test_*.hl` / `test-*.hl` | 19 | |
