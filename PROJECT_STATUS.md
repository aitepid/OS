...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 276（disjoint_set + treap + suffix_array）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 331 | 69 根目录 + 262 内核模块 |
| H-L 总行数 | ~109,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 262 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 730 | shell.hl（+dsu/treap/suffixarray）|
| `test_*.hl` / `test-*.hl` | 19 | |
