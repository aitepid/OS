...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 315（ir + regalloc + codegen）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 370 | 72 根目录 + 301 内核模块 |
| H-L 总行数 | ~109,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 301 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 932 | shell.hl（+ir/regalloc/codegen）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| 验证项 | 结果 |
|---|---|
| `hl-bootstrap build` | ✅ 301 模块编译 + 镜像重建 |
| `validate-workspace` | ✅ 370 HL / 301 模块 / 0 stub |
