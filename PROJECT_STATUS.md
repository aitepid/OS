...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 165（ntp + tar + ui_hexedit + pop3）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 220 | 69 根目录 + 151 内核模块 |
| H-L 总行数 | ~62,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 151 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 195 | shell.hl（+ntp/tar/hexedit/pop3）|
| `test_*.hl` / `test-*.hl` | 19 | |
