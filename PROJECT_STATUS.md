...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 207（base32 + hex + url）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 262 | 69 根目录 + 193 内核模块 |
| H-L 总行数 | ~71,200 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 193 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 397 | shell.hl（+base32/hex/url）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 262 HL / 193 模块 / 0 stub |
