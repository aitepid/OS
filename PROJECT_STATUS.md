...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 192（yaml + msgpack + tftp）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 247 | 69 根目录 + 178 内核模块 |
| H-L 总行数 | ~67,700 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 178 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 324 | shell.hl（+yaml/msgpack/tftp）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 247 HL / 178 模块 / 0 stub |
