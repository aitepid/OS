...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 303（min_cost_flow + centroid + heavy_light）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 358 | 69 根目录 + 289 内核模块 |
| H-L 总行数 | ~105,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 289 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 865 | shell.hl（+mincostflow/centroid/hld）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| 验证项 | 结果 |
|---|---|
| `hl-bootstrap build` | ✅ 289 模块编译 + 镜像重建 |
| `validate-workspace` | ✅ 358 HL / 289 模块 / 0 stub |
