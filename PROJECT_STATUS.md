...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 222（sha256 + dhcp_server + sip）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 277 | 69 根目录 + 208 内核模块 |
| H-L 总行数 | ~74,100 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 208 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 460 | shell.hl（+sha256/dhcpsrv/sip）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 277 HL / 208 模块 / 0 stub |
