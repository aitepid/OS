...
当前仓库处于 `v6.0` 发布线，迭代 117（TCP 回环自测）里程碑。
...
| H-L 总行数 | 47,655 | 根 10,873 + 内核 36,782 |
| `bare-kernel/hl/` 内核模块 | 116 | 编译进 `kernel.bin` |
| `kernel_entry.hl` 行数 | 10,515 | |
| `kernel_entry.hl` 函数数 | 305 | |
| 内核模块总函数数 | 1,503 | 源码定义计数 |
...
| C | `bare-kernel/hl/*.hl`（116 模块，36,782 行） | 内核模块 → `kernel.bin` |
