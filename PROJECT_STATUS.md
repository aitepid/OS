...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 168（irc + ui_calendar + compress）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 223 | 69 根目录 + 154 内核模块 |
| H-L 总行数 | ~66,000 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 154 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 210 | shell.hl（+irc/cal/compress）|
| `test_*.hl` / `test-*.hl` | 19 | |
