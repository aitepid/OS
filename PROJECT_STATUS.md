...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 228（rtp + socks + zlib）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 283 | 69 根目录 + 214 内核模块 |
| H-L 总行数 | ~75,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 214 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 490 | shell.hl（+rtp/socks/zlib）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 283 HL / 214 模块 / 0 stub |
