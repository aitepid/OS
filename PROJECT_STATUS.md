...
| **C: 内核源码** | `bare-kernel/hl/` 114 模块 | ✅ kernel.bin 集成到镜像 | 编译产出 kernel.bin, 1,150+函数, 33条shell命令, 含PCI+VirtIO-blk/net+TCP+FAT16+安装器+内存+任务+鼠标+字体+WM+SMP+AHCI+USB+ACPI+AC97+RTC+微型解释器+SHA-256+HMAC+HKDF |

**当前状态**: 层级 A 有 20+ 个功能已 QEMU 验证（含 shell 20 条命令）；层级 C 的 kernel.bin 已编译并集成到镜像，在 0x120000 运行 33 条独立 shell 命令（含 VirtIO-blk磁盘读写+FAT16格式化与目录列表+TCP/VirtIO-net/内存/任务/鼠标/字体/WM/SMP/AHCI/USB/ACPI+AC97音频/RTC时间+SHA-256+HMAC-SHA-256+HKDF密码学）。A/C 融合正在进行中。
...
| kernel.bin (编译产出) | 1,150+ 函数, 33 shell 命令 |
| QEMU BIOS 引导测试 | **74/74 PASS** (含 8 shell + 4 FAT16 + 6 net + 2 Ring3 + 6 HW + 6 disk + 4 install + 4 run + 3 mem + 3 sha256 + 6 hmac) |
