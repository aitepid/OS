...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 249（curve25519 + aes_gcm + chacha20_poly1305）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 304 | 69 根目录 + 235 内核模块 |
| H-L 总行数 | ~83,000 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 235 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 595 | shell.hl（+curve25519/aesgcm/cp1305）|
| `test_*.hl` / `test-*.hl` | 19 | |
