...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 195（bson + bencode + mime）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 250 | 69 根目录 + 181 内核模块 |
| H-L 总行数 | ~68,400 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 181 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 344 | shell.hl（+bson/bencode/mime）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 250 HL / 181 模块 / 0 stub |
