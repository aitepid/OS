...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 261（attention + bloom_filter + crdt）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 316 | 69 根目录 + 247 内核模块 |
| H-L 总行数 | ~94,500 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 247 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 655 | shell.hl（+attention/bloom/crdt）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 316 HL / 247 模块 / 0 stub |
