...
当前仓库处于 `v6.0` 发布线，最新功能里程碑为迭代 231（lzma + protobuf + chacha20）。
...
| 指标 | 当前值 | 说明 |
|---|---:|---|
| 全部 `.hl` 文件 | 286 | 69 根目录 + 217 内核模块 |
| H-L 总行数 | ~76,200 | 持续增长 |
| `bare-kernel/hl/` 内核模块 | 217 | 编译进 `kernel.bin` |
...
| Shell 命令数 | 505 | shell.hl（+lzma/protobuf/chacha20）|
| `test_*.hl` / `test-*.hl` | 19 | |
...
| `validate-workspace` | ✅ 286 HL / 217 模块 / 0 stub |
