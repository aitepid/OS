...
- `186` 个 `.hl` 文件（48,657 行）：68 根目录 + 118 内核模块
- `22` 个 PowerShell 脚本（9,965 行）
- `hl-bootstrap.hl`：`4,572` 行 | `stdlib.hl`：`1,545` 行
- 内核函数：`1,520` | Shell + 内核命令（去重）：`113`
- 构建/测试主入口：`hl-bootstrap.cmd test`
- 最近完成功能迭代：`120`（内核热补丁 kmod.hl）
