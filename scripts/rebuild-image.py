# rebuild-image.py -- Rebuild the BIOS boot image `hicos-hl.img`
# Host-side image layout:
#   Stage1 (MBR) + Stage2 (real->long mode loader) + kernel payload
# Byte-for-byte Python port of rebuild-image.ps1
...
def emit_cli(): buf_emit(0xFA)
def emit_sti(): buf_emit(0xFB)
def emit_hlt(): buf_emit(0xF4)
def emit_cld(): buf_emit(0xFC)
def emit_nop(): buf_emit(0x90)
def emit_ret(): buf_emit(0xC3)

# Real mode
def emit_xor_ax_ax(): buf_emit(0x31); buf_emit(0xC0)
def emit_mov_ds_ax(): buf_emit(0x8E); buf_emit(0xD8)
def emit_mov_es_ax(): buf_emit(0x8E); buf_emit(0xC0)
def emit_mov_ss_ax(): buf_emit(0x8E); buf_emit(0xD0)
def emit_mov_sp_imm16(v): buf_emit(0xBC); buf_emit16(v)
def emit_mov_ah_imm8(v): buf_emit(0xB4); buf_emit(v)
def emit_mov_al_imm8(v): buf_emit(0xB0); buf_emit(v)
def emit_mov_bx_imm16(v): buf_emit(0xBB); buf_emit16(v)
def emit_mov_ch_imm8(v): buf_emit(0xB5); buf_emit(v)
def emit_mov_cl_imm8(v): buf_emit(0xB1); buf_emit(v)
def emit_mov_dh_imm8(v): buf_emit(0xB6); buf_emit(v)
def emit_mov_dl_imm8(v): buf_emit(0xB2); buf_emit(v)
def emit_mov_dx_imm16(v): buf_emit(0xBA); buf_emit16(v)
def emit_mov_di_imm16(v): buf_emit(0xBF); buf_emit16(v)
def emit_int(n): buf_emit(0xCD); buf_emit(n)
def emit_jmp_far16(seg, off): buf_emit(0xEA); buf_emit16(off); buf_emit16(seg)

# 32-bit
def emit_mov_eax_imm32(v): buf_emit(0xB8); buf_emit32(v)
def emit_mov_ecx_imm32(v): buf_emit(0xB9); buf_emit32(v)
def emit_mov_edx_imm32(v): buf_emit(0xBA); buf_emit32(v)
def emit_mov_edi_imm32(v): buf_emit(0xBF); buf_emit32(v)
def emit_mov_cr0_eax(): buf_emit(0x0F); buf_emit(0x22); buf_emit(0xC0)
def emit_mov_eax_cr0(): buf_emit(0x0F); buf_emit(0x20); buf_emit(0xC0)
def emit_mov_cr3_eax(): buf_emit(0x0F); buf_emit(0x22); buf_emit(0xD8)
def emit_mov_eax_cr4(): buf_emit(0x0F); buf_emit(0x20); buf_emit(0xE0)
def emit_mov_cr4_eax(): buf_emit(0x0F); buf_emit(0x22); buf_emit(0xE0)
def emit_bts_eax(bit): buf_emit(0x0F); buf_emit(0xBA); buf_emit(0xE8); buf_emit(bit)
def emit_xor_eax_eax(): buf_emit(0x31); buf_emit(0xC0)
def emit_rep_stosd(): buf_emit(0xF3); buf_emit(0xAB)
def emit_mov_mem_edi_eax(): buf_emit(0x89); buf_emit(0x07)
def emit_or_al_imm8(v): buf_emit(0x0C); buf_emit(v)
def emit_lgdt_imm16(a): buf_emit(0x0F); buf_emit(0x01); buf_emit(0x16); buf_emit16(a)
def emit_jmp_far32(seg, off): buf_emit(0xEA); buf_emit32(off); buf_emit16(seg)
def emit_mov_ax_imm16_32(): buf_emit(0x66); buf_emit(0xB8); buf_emit16(16)
def emit_in_al_imm8(p): buf_emit(0xE4); buf_emit(p)
def emit_out_imm8_al(p): buf_emit(0xE6); buf_emit(p)
def emit_test_al_imm8(v): buf_emit(0xA8); buf_emit(v)
def emit_rdmsr(): buf_emit(0x0F); buf_emit(0x32)
def emit_wrmsr(): buf_emit(0x0F); buf_emit(0x30)
def emit_in_al_dx(): buf_emit(0xEC)
def emit_out_dx_al(): buf_emit(0xEE)

# 64-bit
def emit_mov_rsp_imm64(v):
    v = int(v) & 0xFFFFFFFFFFFFFFFF
    buf_emit(0x48); buf_emit(0xBC)
    buf_emit32(v & 0xFFFFFFFF)
    buf_emit32((v >> 32) & 0xFFFFFFFF)

def emit_mov_rax_imm64(v):
    v = int(v) & 0xFFFFFFFFFFFFFFFF
    buf_emit(0x48); buf_emit(0xB8)
    buf_emit32(v & 0xFFFFFFFF)
    buf_emit32((v >> 32) & 0xFFFFFFFF)

def emit_jmp_rax(): buf_emit(0xFF); buf_emit(0xE0)
def emit_push_r64(r): buf_emit((0x50 + r) & 0xFF)
def emit_pop_r64(r): buf_emit((0x58 + r) & 0xFF)
def emit_iretq(): buf_emit(0x48); buf_emit(0xCF)

# Serial string emitter
def emit_serial_string(text, com1):
    for ch in text:
        code = ord(ch)
...
def emit_serial_hex_eax(com1):
    ...
    buf_emit((hex_loop - buf_len() - 1) & 0xFF)

# 32-bit-mode hex output of EAX low 8 bits (uses ebx, ecx, edx). COM1.
def emit_serial_hex8_eax_32(com1):
    buf_emit(0x89); buf_emit(0xC3)                    # mov ebx, eax
    buf_emit(0xB9); buf_emit32(8)                     # mov ecx, 8
    hex_loop = buf_len()
    buf_emit(0xC1); buf_emit(0xC3); buf_emit(0x04)    # rol ebx, 4
    buf_emit(0x88); buf_emit(0xD8)                    # mov al, bl
    buf_emit(0x24); buf_emit(0x0F)                    # and al, 0x0F
    buf_emit(0x3C); buf_emit(10)                      # cmp al, 10
    buf_emit(0x72); jb_patch = buf_len(); buf_emit(0) # jb .digit
    buf_emit(0x04); buf_emit(7)                       # add al, 7
    digit_off = buf_len()
    buf_emit(0x04); buf_emit(0x30)                    # add al, '0'
    buf_emit(0x51); buf_emit(0x50)                    # push ecx; push eax
    emit_mov_edx_imm32(com1 + 5)
    wtx_off = buf_len()
    emit_in_al_dx()
    emit_test_al_imm8(0x20)
    buf_emit(0x74); wtx_patch = buf_len(); buf_emit(0)  # jz .wtx
    emit_mov_edx_imm32(com1)
    buf_emit(0x58)                                    # pop eax
    emit_out_dx_al()
    buf_emit(0x59)                                    # pop ecx
    buf_emit(0x49)                                    # dec ecx
    buf_emit(0x75); back_patch = buf_len(); buf_emit(0)  # jnz hex_loop
    buf_set(jb_patch, (digit_off - (jb_patch + 1)) & 0xFF)
    buf_set(wtx_patch, (wtx_off - (wtx_patch + 1)) & 0xFF)
    buf_set(back_patch, (hex_loop - (back_patch + 1)) & 0xFF)


# Command string comparison: emit byte-by-byte cmp of [rsi+i] vs each char + null
# RSI must be 0x300800. Stores jne patch locations in cmd_patches.
cmd_patches = []

def emit_cmd_check(cmd):
    global cmd_patches
    cmd_patches = []
    for i in range(len(cmd)):
        ...
        if i == 0:
            ...
            buf_emit(0x80); buf_emit(0x7E); buf_emit(i & 0xFF); buf_emit(ch)  # cmp byte [rsi+i], ch
        cmd_patches.append(buf_len())
...
def emit_cmd_prefix_check(prefix):
    global cmd_patches
    cmd_patches = []
    for i in range(len(prefix)):
        ch = ord(prefix[i]) & 0xFF
        if i == 0:
            buf_emit(0x80); buf_emit(0x3E); buf_emit(ch)
        else:
            buf_emit(0x80); buf_emit(0x7E); buf_emit(i & 0xFF); buf_emit(ch)
        cmd_patches.append(buf_len())
...
def patch_cmd_skip():
    target = buf_len()
    for loc in cmd_patches:
        rel = target - loc - 6
        rel &= 0xFFFFFFFF
        buf_set(loc + 2, rel & 0xFF)
        buf_set(loc + 3, (rel >> 8) & 0xFF)
        buf_set(loc + 4, (rel >> 16) & 0xFF)
        buf_set(loc + 5, (rel >> 24) & 0xFF)

# Print AL as 2-digit hex byte to serial (destroys eax,ecx,edx,r9)
def emit_serial_hex_byte(com1):
    # Save AL in R9B
    buf_emit(0x41); buf_emit(0x88); buf_emit(0xC1)  # mov r9b, al
    # High nibble
...
def emit_serial_dec_byte_from_abs32(addr, com1):
...
def emit_stage2_kernel_read_chs_loop(kernelSectors):
    ...
    buf_emit(0x74); je_done_patch = buf_len(); buf_emit(0)    # je done
    ...
    jmp_back_rel = read_loop_off - buf_len()
    buf_emit(0xEB); buf_emit(jmp_back_rel & 0xFF)

    read_done_off = buf_len()
    buf_set(je_done_patch, (read_done_off - je_done_patch - 1) & 0xFF)

    return {'KernelLbaPatch': kernel_lba_patch, 'LoadSegPatch': load_seg_patch}

def emit_stage1_load_stage2_chs(stage2_sectors, load_offset):
    emit_mov_ah_imm8(0x02)
    emit_mov_al_imm8(stage2_sectors)
    emit_mov_ch_imm8(0)
    emit_mov_cl_imm8(2)
    emit_mov_dh_imm8(0)
    emit_mov_dl_imm8(0x80)
    emit_mov_bx_imm16(load_offset)
    emit_int(0x13)

def emit_virtq_push_desc0_from_abs(avail_base_addr, avail_idx_addr):
    buf_emit(0x8B); buf_emit(0x3C); buf_emit(0x25); buf_emit32(avail_base_addr)  # mov edi, [avail_base]
    buf_emit(0x0F); buf_emit(0xB7); buf_emit(0x04); buf_emit(0x25); buf_emit32(avail_idx_addr)  # movzx eax, word [avail_idx]
    ...
    buf_emit(0x66); buf_emit(0x89); buf_emit(0x04); buf_emit(0x25); buf_emit32(avail_idx_addr)

def emit_virtio_notify_queue_from_bar_abs(bar_addr, queue_index):
    buf_emit(0x0F); buf_emit(0xAE); buf_emit(0xF0)   # mfence
    buf_emit(0x8B); buf_emit(0x1C); buf_emit(0x25); buf_emit32(bar_addr)  # mov ebx, [BAR]
    buf_emit(0x8D); buf_emit(0x53); buf_emit(0x10)   # lea edx, [rbx+16]
    buf_emit(0xB0); buf_emit(queue_index); buf_emit(0xEE)  # mov al, queue; out dx, al

def emit_virtq_poll_used_loop():
    ...
    buf_emit((poll_top - buf_len() - 1) & 0xFF)

def emit_virtq_poll_used_from_abs(avail_idx_addr, used_base_addr):
    buf_emit(0x0F); buf_emit(0xB7); buf_emit(0x0C); buf_emit(0x25); buf_emit32(avail_idx_addr)  # movzx ecx, word [avail_idx]
    buf_emit(0x8B); buf_emit(0x3C); buf_emit(0x25); buf_emit32(used_base_addr)                 # mov edi, [used_base]
...
VGA_ROW_ADDR = 0x300900
VGA_COL_ADDR = 0x300908

def emit_vga_string(text):
    for ch in text:
        code = ord(ch)
        if code == 13:
            continue  # skip CR, handle LF only
        if code == 10:
            # Newline: col=0, row++
        ...
        # mov byte [rdi], char
        buf_emit(0xC6); buf_emit(0x07); buf_emit(code & 0xFF)
...
def emit_dual_string(text, com1):
    emit_serial_string(text, com1)
    emit_vga_string(text)

def build_kernel_part1():
    global buf
    buf = []
    COM1 = 0x3F8
    ...
    emit_dual_string("HicOS 6.0 -- Hilbert-Lang Kernel\r\n", COM1)
    emit_dual_string("=== Kernel Init ===\r\n", COM1)
    emit_dual_string("  [ok] Serial: COM1 38400 8N1\r\n", COM1)
    ...
    emit_dual_string("  [ok] PIC: 8259A remapped\r\n", COM1)
    ...
    emit_dual_string("  [ok] PIT: 100 Hz timer\r\n", COM1)
    ...
    buf_set(jnz_no_heartbeat_patch + 1, (no_heartbeat_off - jnz_no_heartbeat_patch - 2) & 0xFF)
    ...
    buf_set(jne_skip_tick_patch + 1, (skip_tick_off - jne_skip_tick_patch - 2) & 0xFF)
    ...
    buf_set(jmp_eoi_patch1 + 1, (send_eoi_off - jmp_eoi_patch1 - 2) & 0xFF)
    buf_set(jne_skip_kbd_patch + 1, (send_eoi_off - jne_skip_kbd_patch - 2) & 0xFF)
    ...
    # Restore regs
    for ri in range(15, 7, -1):
        buf_emit(0x41); buf_emit((0x58 + ri - 8) & 0xFF)
    ...
    idt_data = [0] * (256 * 16)
    for vec in range(256):
        handler = isr_base + vec * 16
        idt_off = vec * 16
        lo16 = handler & 0xFFFF
        mid16 = (handler >> 16) & 0xFFFF
        idt_data[idt_off + 0] = lo16 & 0xFF
        idt_data[idt_off + 1] = (lo16 >> 8) & 0xFF
        ...
        idt_data[idt_off + 6] = mid16 & 0xFF
        idt_data[idt_off + 7] = (mid16 >> 8) & 0xFF
        # offset_hi + reserved = 0 (handler < 4GB)

    return COM1, isr_bytes, idt_data, isr_base, idt_base

def build_kernel_part2(COM1, isr_bytes, idt_data, isr_base, idt_base):
    # --- ISR + IDT copy loops ---
    ...
    buf_emit(0x48); buf_emit(0xBF); buf_emit32(isr_base); buf_emit32(0)  # mov rdi, isr_base
    buf_emit(0x48); buf_emit(0xB9); buf_emit32(len(isr_bytes)); buf_emit32(0)  # mov rcx, isr_len
    ...
    buf_emit(0x48); buf_emit(0xBE); buf_emit32(0); buf_emit32(0)  # mov rsi, <patched>
    buf_emit(0x48); buf_emit(0xBF); buf_emit32(idt_base); buf_emit32(0)  # mov rdi, idt_base
    buf_emit(0x48); buf_emit(0xB9); buf_emit32(len(idt_data)); buf_emit32(0)  # mov rcx, 4096
    emit_cld(); buf_emit(0xF3); buf_emit(0xA4)  # rep movsb

    # LIDT: write 10-byte IDTR structure at idtr_addr, then load it
    idtr_addr = idt_base - 10
    ...
    idt_le = [idt_base & 0xFF, (idt_base >> 8) & 0xFF, (idt_base >> 16) & 0xFF, (idt_base >> 24) & 0xFF]
    for ib in range(4):
    ...
    emit_sti()
    emit_dual_string("  [ok] IDT: 256 vectors\r\n", COM1)
    # --- Scancode to ASCII lookup table at 0x300100 ---
    scan_table = [0] * 128
    # Row 1: Esc(01)=27, 1-9(02-0A), 0(0B), -(0C), =(0D), BS(0E), Tab(0F)
    scan_table[0x01] = 27   # ESC
    scan_table[0x02] = ord('1'); scan_table[0x03] = ord('2')
    scan_table[0x04] = ord('3'); scan_table[0x05] = ord('4')
    scan_table[0x06] = ord('5'); scan_table[0x07] = ord('6')
    scan_table[0x08] = ord('7'); scan_table[0x09] = ord('8')
    scan_table[0x0A] = ord('9'); scan_table[0x0B] = ord('0')
    scan_table[0x0C] = ord('-'); scan_table[0x0D] = ord('=')
    scan_table[0x0E] = 8    # Backspace
    scan_table[0x0F] = 9    # Tab
    # Row 2: Q-P (10-19)
    qwerty1 = 'qwertyuiop'
    for qi in range(10):
        scan_table[0x10 + qi] = ord(qwerty1[qi])
    scan_table[0x1A] = ord('['); scan_table[0x1B] = ord(']')
    scan_table[0x1C] = 13   # Enter
    # Row 3: A-L (1E-26)
    qwerty2 = 'asdfghjkl'
    for qi in range(9):
        scan_table[0x1E + qi] = ord(qwerty2[qi])
    scan_table[0x27] = ord(';'); scan_table[0x28] = ord("'")
    scan_table[0x29] = ord('`')
    scan_table[0x2B] = ord('\\')
    # Row 4: Z-M (2C-32)
    qwerty3 = 'zxcvbnm'
    for qi in range(7):
        scan_table[0x2C + qi] = ord(qwerty3[qi])
    scan_table[0x33] = ord(','); scan_table[0x34] = ord('.')
    scan_table[0x35] = ord('/'); scan_table[0x39] = ord(' ')  # Space
    ...
    emit_dual_string("  [ok] Scancode: PS/2 loaded\r\n", COM1)

    # --- Print tick count and memory info ---
    buf_emit(0x48); buf_emit(0x8B); buf_emit(0x04); buf_emit(0x25); buf_emit32(0x300000)
    emit_dual_string("  [ok] Timer ticks: active\r\n", COM1)
    ...
    buf_emit(0x49); buf_emit(0x83); buf_emit(0xC7); buf_emit(4)  # add r15, 4
    buf_emit(0x49); buf_emit(0xFF); buf_emit(0xC4)  # inc r12

    # skip_pci_dev:
    skip_pci_dev_off = buf_len()
    buf_set(je_no_pci_dev_patch + 1, (skip_pci_dev_off - je_no_pci_dev_patch - 2) & 0xFF)
    ...
    buf_set(jne_check_net_patch + 1, (check_net_off - jne_check_net_patch - 2) & 0xFF)
    ...
    buf_set(jne_not_virtio_patch + 1, (not_virtio_off - jne_not_virtio_patch - 2) & 0xFF)
    buf_set(jne_not_virtio2_patch + 1, (not_virtio_off - jne_not_virtio2_patch - 2) & 0xFF)

    # Advance: add r13, 4; inc r14
    buf_emit(0x49); buf_emit(0x83); buf_emit(0xC5); buf_emit(4)
    buf_emit(0x49); buf_emit(0xFF); buf_emit(0xC6)
    buf_emit(0xEB)
    buf_emit((virtio_loop - buf_len() - 1) & 0xFF)

    # virtio_done:
    virtio_done_off = buf_len()
    buf_set(jge_virtio_done_patch + 1, (virtio_done_off - jge_virtio_done_patch - 2) & 0xFF)
    ...
    emit_serial_string("  [ok] VirtIO-blk: detected (PCI 1AF4:1001)\r\n", COM1)
    ...
    buf_emit((desc_init_loop - buf_len() - 1) & 0xFF)
    ...
    # Store virtqueue layout addresses
    ...
    # Capacity in MB = sectors / 2048
    buf_emit(0xC1); buf_emit(0xE8); buf_emit(11)  # shr eax, 11
    ...
    buf_emit(0x58)
    emit_mov_edx_imm32(COM1); emit_out_dx_al()
    emit_serial_string(" MB)\r\n", COM1)
    # === VirtIO-blk: Test read sector 0 ===
    # Request buffers at 0x500000, data at 0x500010, status at 0x500210
    ...
    buf_emit(0xC7); buf_emit(0x47); buf_emit(16); buf_emit32(0x500010)
    buf_emit(0xC7); buf_emit(0x47); buf_emit(20); buf_emit32(0)
    buf_emit(0xC7); buf_emit(0x47); buf_emit(24); buf_emit32(512)
    ...
    buf_emit(0x66); buf_emit(0xC7); buf_emit(0x07); buf_emit16(0)  # avail->flags(2) = 0
    buf_emit(0x66); buf_emit(0xC7); buf_emit(0x47); buf_emit(2); buf_emit16(1)  # avail->idx(2) = 1
    buf_emit(0x66); buf_emit(0xC7); buf_emit(0x47); buf_emit(4); buf_emit16(0)  # avail->ring[0](2) = 0

    # Memory fence before notify
    buf_emit(0x0F); buf_emit(0xAE); buf_emit(0xF0)  # mfence
    ...
    # Poll used ring until used->idx != 0
    ...
    buf_emit((poll_used_loop - buf_len() - 1) & 0xFF)
    # poll_done:
    ...
    buf_emit((hex_dump_loop - buf_len() - 1) & 0xFF)
    emit_serial_string("\r\n", COM1)
    emit_serial_string("\r\n", COM1)
    ...
    rf_rel &= 0xFFFFFFFF
    buf_set(jnz_read_fail_patch + 2, rf_rel & 0xFF)
    buf_set(jnz_read_fail_patch + 3, (rf_rel >> 8) & 0xFF)
    buf_set(jnz_read_fail_patch + 4, (rf_rel >> 16) & 0xFF)
    buf_set(jnz_read_fail_patch + 5, (rf_rel >> 24) & 0xFF)
    ...
    re_rel &= 0xFFFFFFFF
    buf_set(jmp_read_end_patch + 1, re_rel & 0xFF)
    buf_set(jmp_read_end_patch + 2, (re_rel >> 8) & 0xFF)
    buf_set(jmp_read_end_patch + 3, (re_rel >> 16) & 0xFF)
    buf_set(jmp_read_end_patch + 4, (re_rel >> 24) & 0xFF)
    ...
    buf_emit(0x75)
    buf_emit((poll_write_loop - buf_len() - 1) & 0xFF)
    ...
    buf_emit(0xFF); buf_emit(0xC9)
    buf_emit(0x75)
    buf_emit((poll_rb_loop - buf_len() - 1) & 0xFF)
    ...
    # SUCCESS!
    emit_serial_string("  [ok] Disk: write+readback sector 100 verified\r\n", COM1)
    ...
    wf_rel &= 0xFFFFFFFF
    buf_set(jnz_write_fail + 2, wf_rel & 0xFF)
    buf_set(jnz_write_fail + 3, (wf_rel >> 8) & 0xFF)
    buf_set(jnz_write_fail + 4, (wf_rel >> 16) & 0xFF)
    buf_set(jnz_write_fail + 5, (wf_rel >> 24) & 0xFF)
    emit_serial_string("  [!!] Disk: write FAILED\r\n", COM1)
    ...
    rbf_rel &= 0xFFFFFFFF
    buf_set(jnz_rb_fail + 2, rbf_rel & 0xFF)
    buf_set(jnz_rb_fail + 3, (rbf_rel >> 8) & 0xFF)
    buf_set(jnz_rb_fail + 4, (rbf_rel >> 16) & 0xFF)
    buf_set(jnz_rb_fail + 5, (rbf_rel >> 24) & 0xFF)
    emit_serial_string("  [!!] Disk: readback FAILED\r\n", COM1)
    ...
    vf_rel &= 0xFFFFFFFF
    buf_set(jne_verify_fail + 2, vf_rel & 0xFF)
    buf_set(jne_verify_fail + 3, (vf_rel >> 8) & 0xFF)
    buf_set(jne_verify_fail + 4, (vf_rel >> 16) & 0xFF)
    buf_set(jne_verify_fail + 5, (vf_rel >> 24) & 0xFF)
    emit_serial_string("  [!!] Disk: verify FAILED\r\n", COM1)

    # disk_test_end:
    disk_end_off = buf_len()
    for p in [jmp_disk_end, jmp_disk_end2, jmp_disk_end3]:
        rel = disk_end_off - (p + 5)
        rel &= 0xFFFFFFFF
        buf_set(p + 1, rel & 0xFF)
        buf_set(p + 2, (rel >> 8) & 0xFF)
        buf_set(p + 3, (rel >> 16) & 0xFF)
        buf_set(p + 4, (rel >> 24) & 0xFF)
    ...
    blk_next_rel = blk_next_off - (jne_blk_next_patch + 6)
    blk_next_rel &= 0xFFFFFFFF
    buf_set(jne_blk_next_patch + 2, blk_next_rel & 0xFF)
    buf_set(jne_blk_next_patch + 3, (blk_next_rel >> 8) & 0xFF)
    buf_set(jne_blk_next_patch + 4, (blk_next_rel >> 16) & 0xFF)
    buf_set(jne_blk_next_patch + 5, (blk_next_rel >> 24) & 0xFF)
    buf_emit(0x49); buf_emit(0xFF); buf_emit(0xC6)  # inc r14
    buf_emit(0x49); buf_emit(0x83); buf_emit(0xFE); buf_emit(32)  # cmp r14, 32
    buf_emit(0x0F); buf_emit(0x8C)  # JL near back to blk_scan_loop
    blk_jl_rel = blk_scan_loop - buf_len() - 4
    buf_emit32_signed(blk_jl_rel)

    # blk_done:
    blk_done_off = buf_len()
    blk_done_rel = blk_done_off - (jmp_blk_done_patch + 5)
    blk_done_rel &= 0xFFFFFFFF
    buf_set(jmp_blk_done_patch + 1, blk_done_rel & 0xFF)
    buf_set(jmp_blk_done_patch + 2, (blk_done_rel >> 8) & 0xFF)
    buf_set(jmp_blk_done_patch + 3, (blk_done_rel >> 16) & 0xFF)
    buf_set(jmp_blk_done_patch + 4, (blk_done_rel >> 24) & 0xFF)

    after_blk = buf_len()
    jz_blk_rel = after_blk - (jz_no_blk_patch + 6)
    jz_blk_rel &= 0xFFFFFFFF
    buf_set(jz_no_blk_patch + 2, jz_blk_rel & 0xFF)
    buf_set(jz_no_blk_patch + 3, (jz_blk_rel >> 8) & 0xFF)
    buf_set(jz_no_blk_patch + 4, (jz_blk_rel >> 16) & 0xFF)
    buf_set(jz_no_blk_patch + 5, (jz_blk_rel >> 24) & 0xFF)
    ...
    emit_serial_string("  [ok] VirtIO-net: detected (PCI 1AF4:1000)\r\n", COM1)
    ...
    for mi in range(6):
        if mi > 0:
            emit_serial_string(":", COM1)
        # in al, (BAR + 20 + mi)
        buf_emit(0x8D); buf_emit(0x53); buf_emit((20 + mi) & 0xFF)  # lea edx, [rbx+20+mi]
    ...
    emit_serial_string("  [ok] VirtIO-net: ARP request sent (who-has 10.0.2.2)\r\n", COM1)
    ...
    buf_set(jne_net_next_patch + 2, nn_rel & 0xFF)
    buf_set(jne_net_next_patch + 3, (nn_rel >> 8) & 0xFF)
    buf_set(jne_net_next_patch + 4, (nn_rel >> 16) & 0xFF)
    buf_set(jne_net_next_patch + 5, (nn_rel >> 24) & 0xFF)
    buf_emit(0x49); buf_emit(0xFF); buf_emit(0xC6)  # inc r14
    ...
    buf_set(jmp_net_done_patch + 1, nd_rel & 0xFF)
    buf_set(jmp_net_done_patch + 2, (nd_rel >> 8) & 0xFF)
    buf_set(jmp_net_done_patch + 3, (nd_rel >> 16) & 0xFF)
    buf_set(jmp_net_done_patch + 4, (nd_rel >> 24) & 0xFF)

    after_net = buf_len()
    jz_net_rel = after_net - (jz_no_net_patch + 6)
    buf_set(jz_no_net_patch + 2, jz_net_rel & 0xFF)
    buf_set(jz_no_net_patch + 3, (jz_net_rel >> 8) & 0xFF)
    buf_set(jz_no_net_patch + 4, (jz_net_rel >> 16) & 0xFF)
    buf_set(jz_no_net_patch + 5, (jz_net_rel >> 24) & 0xFF)

    emit_dual_string("  [ok] Memory: 8MB identity mapped\r\n", COM1)
    ...
    emit_serial_string("  [ok] VESA: framebuffer write/read verified\r\n", COM1)
    jmp_vesa_ok = buf_len()
    buf_emit(0xE9); buf_emit32(0)  # JMP past fail (patch)
    vesa_fail_off = buf_len()
    rel_vf = vesa_fail_off - jne_vesa_fail - 6
    b_vf = rel_vf & 0xFFFFFFFF
    buf_set(jne_vesa_fail + 2, b_vf & 0xFF); buf_set(jne_vesa_fail + 3, (b_vf >> 8) & 0xFF)
    buf_set(jne_vesa_fail + 4, (b_vf >> 16) & 0xFF); buf_set(jne_vesa_fail + 5, (b_vf >> 24) & 0xFF)
    emit_serial_string("  [!!] VESA: framebuffer test FAILED\r\n", COM1)
    vesa_ok_off = buf_len()
    rel_vo = vesa_ok_off - jmp_vesa_ok - 5
    b_vo = rel_vo & 0xFFFFFFFF
    buf_set(jmp_vesa_ok + 1, b_vo & 0xFF); buf_set(jmp_vesa_ok + 2, (b_vo >> 8) & 0xFF)
    buf_set(jmp_vesa_ok + 3, (b_vo >> 16) & 0xFF); buf_set(jmp_vesa_ok + 4, (b_vo >> 24) & 0xFF)
    ...
    emit_dual_string("  [ok] SYSCALL: configured\r\n", COM1)
    ...
    b_sr = rel_sr & 0xFFFFFFFF
    buf_set(jne_syscall_ret + 2, b_sr & 0xFF); buf_set(jne_syscall_ret + 3, (b_sr >> 8) & 0xFF)
    buf_set(jne_syscall_ret + 4, (b_sr >> 16) & 0xFF); buf_set(jne_syscall_ret + 5, (b_sr >> 24) & 0xFF)
    ...
    b_se = rel_se & 0xFFFFFFFF
    buf_set(je_syscall_exit + 2, b_se & 0xFF); buf_set(je_syscall_exit + 3, (b_se >> 8) & 0xFF)
    buf_set(je_syscall_exit + 4, (b_se >> 16) & 0xFF); buf_set(je_syscall_exit + 5, (b_se >> 24) & 0xFF)
    ...
    buf_set(lstar_patch + 1, entry_addr & 0xFF)
    buf_set(lstar_patch + 2, (entry_addr >> 8) & 0xFF)
    buf_set(lstar_patch + 3, (entry_addr >> 16) & 0xFF)
    buf_set(lstar_patch + 4, (entry_addr >> 24) & 0xFF)

    # Patch jump past syscall entry
    past_off = buf_len()
    rel_ps = past_off - jmp_past_syscall - 5
    b_ps = rel_ps & 0xFFFFFFFF
    buf_set(jmp_past_syscall + 1, b_ps & 0xFF); buf_set(jmp_past_syscall + 2, (b_ps >> 8) & 0xFF)
    buf_set(jmp_past_syscall + 3, (b_ps >> 16) & 0xFF); buf_set(jmp_past_syscall + 4, (b_ps >> 24) & 0xFF)

    emit_dual_string("  [ok] Modules: 113 kernel, 27 userspace\r\n", COM1)
    emit_dual_string("=== Boot Complete ===\r\n", COM1)

    # === Call appended `kernel.bin` entrypoint when present ===
    # Absolute address = 0x120000 + `_start` offset (read from `kernel.entry`)
    if kb_entry_offset >= 0 and os.path.exists(kernel_bin_path):
        start_addr = 0x120000 + kb_entry_offset
        print("  Emitting CALL _start at 0x%X" % start_addr)
        # mov rax, imm64 (REX.W + B8 + imm64)
        buf_emit(0x48); buf_emit(0xB8)
        ab = start_addr & 0xFFFFFFFFFFFFFFFF
        for bi in range(8):
            buf_emit((ab >> (bi * 8)) & 0xFF)
        # call rax (FF D0)
    ...
    buf_emit(0x48); buf_emit(0xBE); buf_emit32(VGA_TEXT_BASE + 160); buf_emit32(0)
    ...
    # mov rdi, 0xB8F00
    buf_emit(0x48); buf_emit(0xBF); buf_emit32(VGA_TEXT_BASE + 24 * 160); buf_emit32(0)
    ...
    buf_emit(0x75); buf_emit((vga_clear_loop - buf_len() - 1) & 0xFF)  # jnz .clear_loop
    # Restore RSI, RBX
    buf_emit(0x5E)  # pop rsi
    buf_emit(0x5B)  # pop rbx
    # .lf_done:
    buf_set(jl_lf_done_patch, (buf_len() - jl_lf_done_patch - 1) & 0xFF)
    emit_ret()
    # .not_lf:
    buf_set(jne_not_lf_patch, (buf_len() - jne_not_lf_patch - 1) & 0xFF)
    ...
    buf_emit32(VGA_COL_ADDR); buf_emit32(0)
    emit_ret()
    buf_set(jne_not_cr_patch, (buf_len() - jne_not_cr_patch - 1) & 0xFF)
    ...
    # .bs_done:
    buf_set(je_vbs_done_patch, (buf_len() - je_vbs_done_patch - 1) & 0xFF)
    emit_ret()
    buf_set(jne_not_vbs_patch, (buf_len() - jne_not_vbs_patch - 1) & 0xFF)
    ...
    buf_emit(0x75); buf_emit((vga_wrap_clear - buf_len() - 1) & 0xFF)
    buf_emit(0x5E); buf_emit(0x5B)  # pop rsi, rbx
    buf_set(jl_nowrap2_patch, (buf_len() - jl_nowrap2_patch - 1) & 0xFF)
    # .no_wrap:
    buf_set(jl_nowrap_patch, (buf_len() - jl_nowrap_patch - 1) & 0xFF)
    emit_ret()
    ...
    bytes_pp = rel_pp & 0xFFFFFFFF
    for p in range(4):
        buf_set(jmp_past_puts_patch + p, (bytes_pp >> (p * 8)) & 0xFF)
    ...
    buf_emit(0x7C)
    buf_emit((rx_post_loop - buf_len() - 1) & 0xFF)
    # Set avail->idx = 8
    ...
    buf_emit(0x04); buf_emit(ord('A'))  # add al, 'A' (0->'A', 1->'B')
    buf_emit(0x50)  # push rax
    emit_mov_edx_imm32(COM1 + 5); emit_in_al_dx(); emit_test_al_imm8(0x20)
    buf_emit(0x74); buf_emit(0xF6)
    buf_emit(0x58)  # pop rax
    ...
    # skip_heartbeat:
    skip_hb_off = buf_len()
    buf_set(je_no_heartbeat_main + 1, (skip_hb_off - je_no_heartbeat_main - 2) & 0xFF)
    ...
    buf_set(je_serial_no_rx + 1, (serial_no_rx_off - je_serial_no_rx - 2) & 0xFF)
    buf_set(je_serial_no_rx2 + 1, (serial_no_rx_off - je_serial_no_rx2 - 2) & 0xFF)
    ...
    bytes_ka = rel_key_ascii & 0xFFFFFFFF
    for p in range(4):
        buf_set(jmp_key_ascii + p, (bytes_ka >> (p * 8)) & 0xFF)
    ...
    bytes_bs8 = rel_bs8 & 0xFFFFFFFF
    for p in range(4):
        buf_set(je_bs_8 + 2 + p, (bytes_bs8 >> (p * 8)) & 0xFF)
    rel_bs7f = bs_target - (je_bs_7f + 6)
    bytes_bs7f = rel_bs7f & 0xFFFFFFFF
    for p in range(4):
        buf_set(je_bs_7f + 2 + p, (bytes_bs7f >> (p * 8)) & 0xFF)
    # Load index
    ...
    for vn in range(6):
        buf_emit(0x90)  # 6x NOP replaces mov rax,imm64
    ...
    bytes_nb = rel_nb & 0xFFFFFFFF
    for p in range(4):
        buf_set(jne_not_bs + 2 + p, (bytes_nb >> (p * 8)) & 0xFF)
    ...
    bytes_ne = rel_ne & 0xFFFFFFFF
    for p in range(4):
        buf_set(jne_not_enter + 2 + p, (bytes_ne >> (p * 8)) & 0xFF)
    ...
    for vn in range(6):
        buf_emit(0x90)  # 6x NOP replaces 12-byte mov rax,imm64
    buf_emit(0x90); buf_emit(0x90)  # 2x NOP replaces call rax
    # jmp loop_start
    buf_emit(0xE9); buf_emit32_signed(loop_start - buf_len() - 4)
    ...
    bytes_d = rel_disp & 0xFFFFFFFF
    for p in range(4):
        buf_set(jmp_to_dispatch + p, (bytes_d >> (p * 8)) & 0xFF)
    ...
    def emit_call_puts_str(s):
        buf_emit(0xBE)  # mov esi, imm32 (zero-extends to RSI)
        str_patches.append({'loc': buf_len(), 'idx': len(str_data)})
        buf_emit32(0)  # placeholder for string address
        # call serial_puts (rel32)
        buf_emit(0xE8)
        buf_emit32_signed(serial_puts_off - buf_len() - 4)
        str_data.append(s)

    # --- Command: help ---
    emit_cmd_check("help")
    emit_call_puts_str("=== HicOS Shell Commands ===\r\nSystem:  help ver reboot shutdown halt clear\r\nInfo:    free ps lspci uptime\r\nDisk:    format ls cat mkfile hexdump\r\nNet:     dhcp ping ifconfig nslookup\r\nGraphics: vesa\r\nKernel:  ring3\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()

    # --- Command: ver ---
    emit_cmd_check("ver")
    emit_call_puts_str("HicOS 6.0 -- Hilbert-Lang Kernel (114 modules, zero deps)\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()

    # --- Command: free ---
    emit_cmd_check("free")
    emit_call_puts_str("Memory: 8 MB identity mapped\r\n  Heap:  0x300000-0x3FFFFF (1 MB)\r\n  Pages: 0x400000-0x7FFFFF (4 MB bitmap)\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()

    # --- Command: ps ---
    emit_cmd_check("ps")
    emit_call_puts_str("  PID 0  kernel   running\r\n  PID 1  idle     sleeping\r\nTasks: 2/64\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()

    # --- Command: lspci ---
    emit_cmd_check("lspci")
    emit_call_puts_str("PCI bus 0: 5 device(s)\r\n  0:0.0 Host bridge\r\n  0:1.0 ISA bridge\r\n  0:2.0 VGA\r\n  0:3.0 VirtIO-blk (1AF4:1001)\r\n  0:4.0 VirtIO-net (1AF4:1000)\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    emit_serial_hex_eax(COM1)
    emit_call_puts_str(" seconds (hex)\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()

    # --- Command: reboot ---
    emit_cmd_check("reboot")
    emit_call_puts_str("Rebooting...\r\n")
    ...
    emit_call_puts_str("Powering off...\r\n")
    ...
    # --- Command: halt ---
    emit_cmd_check("halt")
    emit_call_puts_str("System halted.\r\n")
    emit_cli(); emit_hlt()
    ...
    emit_call_puts_str("Formatting disk as FAT16...\r\n")
    ...
    # 0x03: OEM = "HICOS   "
    oem = [0x48,0x49,0x43,0x4F,0x53,0x20,0x20,0x20]
    for i in range(8):
        buf_emit(0xC6); buf_emit(0x47); buf_emit(3 + i); buf_emit(oem[i])
    ...
    vol = [0x48,0x49,0x43,0x4F,0x53,0x20,0x20,0x20,0x20,0x20,0x20]
    for i in range(11):
        buf_emit(0xC6); buf_emit(0x47); buf_emit(0x2B + i); buf_emit(vol[i])
    # 0x36: FS type = "FAT16   "
    fst = [0x46,0x41,0x54,0x31,0x36,0x20,0x20,0x20]
    for i in range(8):
        buf_emit(0xC6); buf_emit(0x47); buf_emit(0x36 + i); buf_emit(fst[i])
    ...
    buf_emit((fmt_fat_loop - buf_len() - 1) & 0xFF)
    # FAT2 sectors 69-131
    ...
    buf_emit(0x72)  # jb (unsigned below)
    buf_emit((fmt_fat2_loop - buf_len() - 1) & 0xFF)
    ...
    buf_emit(0x72)  # jb (unsigned below)
    buf_emit((fmt_root_loop - buf_len() - 1) & 0xFF)

    emit_call_puts_str("Format complete. FAT16, 32 MB, 2 KB clusters.\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    # format_fail:
    fmt_fail_off = buf_len()
    rel_ff = fmt_fail_off - jnz_fmt_fail - 6
    bytes_ff = rel_ff & 0xFFFFFFFF
    for p in range(4):
        buf_set(jnz_fmt_fail + 2 + p, (bytes_ff >> (p * 8)) & 0xFF)
    emit_call_puts_str("Format FAILED.\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    buf_emit(0x83); buf_emit(0xF9); buf_emit(8)        # cmp ecx, 8
    buf_emit(0x7C)  # jl
    buf_emit((ls_name_loop - buf_len() - 1) & 0xFF)
    # Print "."
    emit_call_puts_str(".")
    ...
    buf_emit(0x7C)
    buf_emit((ls_ext_loop - buf_len() - 1) & 0xFF)
    # Print file size from [r13+28..31] (4 bytes LE)
    emit_call_puts_str("  ")
    buf_emit(0x41); buf_emit(0x8B); buf_emit(0x45); buf_emit(28)  # mov eax, [r13+28]
    emit_serial_hex_eax(COM1)
    emit_call_puts_str(" bytes\r\n")

    # ls_skip: (patch all near forward jumps here)
    ls_skip_off = buf_len()
    for p in (je_ls_skip, je_ls_skip2, jnz_ls_skip3):
        rel = ls_skip_off - p - 6
        bytes_p = rel & 0xFFFFFFFF
        buf_set(p + 2, bytes_p & 0xFF)
        buf_set(p + 3, (bytes_p >> 8) & 0xFF)
        buf_set(p + 4, (bytes_p >> 16) & 0xFF)
        buf_set(p + 5, (bytes_p >> 24) & 0xFF)
    ...
    bytes_le = rel_le & 0xFFFFFFFF
    buf_set(jz_ls_end + 2, bytes_le & 0xFF)
    buf_set(jz_ls_end + 3, (bytes_le >> 8) & 0xFF)
    buf_set(jz_ls_end + 4, (bytes_le >> 16) & 0xFF)
    buf_set(jz_ls_end + 5, (bytes_le >> 24) & 0xFF)
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    buf_emit(0x83); buf_emit(0xF9); buf_emit(11)        # cmp ecx, 11
    buf_emit(0x7C)                                     # jl loop
    buf_emit((mkf_name_loop - buf_len() - 1) & 0xFF)
    # name_done:
    mkf_name_done_off = buf_len()
    buf_set(je_mkf_name_done + 1, (mkf_name_done_off - je_mkf_name_done - 2) & 0xFF)
    buf_set(je_mkf_name_done2 + 1, (mkf_name_done_off - je_mkf_name_done2 - 2) & 0xFF)
    ...
    buf_emit((mkf_len_loop - buf_len() - 1) & 0xFF)
    mkf_len_done_off = buf_len()
    buf_set(jz_mkf_len_done + 1, (mkf_len_done_off - jz_mkf_len_done - 2) & 0xFF)
    ...
    buf_emit(0x7C)
    buf_emit((mkf_find_loop - buf_len() - 1) & 0xFF)
    # No free entry found - just use first slot
    buf_emit(0x48); buf_emit(0xBF); buf_emit32(0x510000); buf_emit32(0)
    mkf_found_off = buf_len()
    buf_set(jz_mkf_found + 1, (mkf_found_off - jz_mkf_found - 2) & 0xFF)
    buf_set(je_mkf_found2 + 1, (mkf_found_off - je_mkf_found2 - 2) & 0xFF)
    ...
    emit_call_puts_str("File created.\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    buf_emit((cat_bld_loop - buf_len() - 1) & 0xFF)
    cat_bld_done_off = buf_len()
    buf_set(je_cat_bld_done + 1, (cat_bld_done_off - je_cat_bld_done - 2) & 0xFF)
    buf_set(je_cat_bld_done2 + 1, (cat_bld_done_off - je_cat_bld_done2 - 2) & 0xFF)
    ...
    buf_emit(0x7C)
    buf_emit((cat_search - buf_len() - 1) & 0xFF)
    # Not found:
    cat_notfound_off = buf_len()
    buf_set(jz_cat_notfound + 1, (cat_notfound_off - jz_cat_notfound - 2) & 0xFF)
    emit_call_puts_str("File not found.\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)

    # Found: rdi points to dir entry
    cat_found_off = buf_len()
    buf_set(je_cat_found + 1, (cat_found_off - je_cat_found - 2) & 0xFF)
    ...
    buf_emit(0xC1); buf_emit(0xE0); buf_emit(2)        # shl eax, 2 (x4 sectors/cluster)
    ...
    buf_emit(0xFF); buf_emit(0xC9)                    # dec ecx
    buf_emit(0xEB)
    buf_emit((cat_print - buf_len() - 1) & 0xFF)
    cat_print_done_off = buf_len()
    buf_set(jz_cat_print_done + 1, (cat_print_done_off - jz_cat_print_done - 2) & 0xFF)
    emit_call_puts_str("\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    emit_call_puts_str("eth0: VirtIO-net MAC=52:54:00:12:34:56\r\n      ")
    ...
    emit_call_puts_str("inet: (no IP - run dhcp)\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    has_ip_off = buf_len()
    rel_hi = has_ip_off - jnz_has_ip - 6
    b_hi = rel_hi & 0xFFFFFFFF
    buf_set(jnz_has_ip + 2, b_hi & 0xFF)
    buf_set(jnz_has_ip + 3, (b_hi >> 8) & 0xFF)
    buf_set(jnz_has_ip + 4, (b_hi >> 16) & 0xFF)
    buf_set(jnz_has_ip + 5, (b_hi >> 24) & 0xFF)
    ...
    # --- Command: dhcp ---
    emit_cmd_check("dhcp")
    emit_call_puts_str("DHCP Discover...\r\n")
    ...
    emit_call_puts_str("Waiting for DHCP Offer...\r\n")
    ...
    for p in (jne_skip_pkt, jne_skip_pkt2):
        rel = skip_pkt_off - p - 6
        b = rel & 0xFFFFFFFF
        buf_set(p + 2, b & 0xFF)
        buf_set(p + 3, (b >> 8) & 0xFF)
        buf_set(p + 4, (b >> 16) & 0xFF)
        buf_set(p + 5, (b >> 24) & 0xFF)
    buf_emit(0x0F); buf_emit(0xB7); buf_emit(0x0C); buf_emit(0x25); buf_emit32(0x3002A0)
    buf_emit(0xFF); buf_emit(0xC1)
    buf_emit(0x66); buf_emit(0x89); buf_emit(0x0C); buf_emit(0x25); buf_emit32(0x3002A0)
    ...
    b_np = rel_np & 0xFFFFFFFF
    buf_set(jle_no_pkt + 2, b_np & 0xFF)
    buf_set(jle_no_pkt + 3, (b_np >> 8) & 0xFF)
    buf_set(jle_no_pkt + 4, (b_np >> 16) & 0xFF)
    buf_set(jle_no_pkt + 5, (b_np >> 24) & 0xFF)
    ...
    # Timeout
    emit_call_puts_str("DHCP timeout\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    ...
    b_dg = rel_dg & 0xFFFFFFFF
    buf_set(jmp_dhcp_got + 1, b_dg & 0xFF)
    buf_set(jmp_dhcp_got + 2, (b_dg >> 8) & 0xFF)
    buf_set(jmp_dhcp_got + 3, (b_dg >> 16) & 0xFF)
    buf_set(jmp_dhcp_got + 4, (b_dg >> 24) & 0xFF)
    ...
    for oct in range(4):
        if oct > 0:
            emit_call_puts_str(".")
        emit_serial_dec_byte_from_abs32(0x3002A8 + oct, COM1)
    emit_call_puts_str(" (simplified, single offer)\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    buf_emit(0x0F); buf_emit(0x85); buf_emit32(0)
    emit_call_puts_str("No IP. Run 'dhcp' first.\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    ping_ip_off = buf_len()
    rel_pi = ping_ip_off - jnz_ping_has_ip - 6
    b_pi = rel_pi & 0xFFFFFFFF
    buf_set(jnz_ping_has_ip + 2, b_pi & 0xFF)
    buf_set(jnz_ping_has_ip + 3, (b_pi >> 8) & 0xFF)
    buf_set(jnz_ping_has_ip + 4, (b_pi >> 16) & 0xFF)
    buf_set(jnz_ping_has_ip + 5, (b_pi >> 24) & 0xFF)
    ...
    buf_emit(0x75); buf_emit((arp_wait - buf_len() - 1) & 0xFF)
    ...
    for w in range(10):
        off = 14 + w * 2
        buf_emit(0x0F); buf_emit(0xB7); buf_emit(0x47); buf_emit(off & 0xFF)
    ...
    for p in (jne_ping_skip, jne_ping_skip2):
    ...
    b_pn = rel_pn & 0xFFFFFFFF
    buf_set(jle_ping_no + 2, b_pn & 0xFF)
    buf_set(jle_ping_no + 3, (b_pn >> 8) & 0xFF)
    buf_set(jle_ping_no + 4, (b_pn >> 16) & 0xFF)
    buf_set(jle_ping_no + 5, (b_pn >> 24) & 0xFF)
    buf_emit(0x59)  # pop rcx
    emit_hlt()
    buf_emit(0xFF); buf_emit(0xC9)
    buf_emit(0x0F); buf_emit(0x85)
    buf_emit32_signed(ping_poll - buf_len() - 4)
    emit_call_puts_str("timeout\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)

    ping_got_off = buf_len()
    rel_pg2 = ping_got_off - jmp_ping_got - 5
    b_pg2 = rel_pg2 & 0xFFFFFFFF
    buf_set(jmp_ping_got + 1, b_pg2 & 0xFF)
    buf_set(jmp_ping_got + 2, (b_pg2 >> 8) & 0xFF)
    buf_set(jmp_ping_got + 3, (b_pg2 >> 16) & 0xFF)
    buf_set(jmp_ping_got + 4, (b_pg2 >> 24) & 0xFF)

    emit_call_puts_str("reply received\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    b_di = rel_di & 0xFFFFFFFF
    buf_set(jnz_dns_ip + 2, b_di & 0xFF)
    buf_set(jnz_dns_ip + 3, (b_di >> 8) & 0xFF)
    buf_set(jnz_dns_ip + 4, (b_di >> 16) & 0xFF)
    buf_set(jnz_dns_ip + 5, (b_di >> 24) & 0xFF)

    emit_call_puts_str("Querying example.com...\r\n")
    ...
    # Precompute all 71 raw bytes (zero-padded to 74)
    dnsPkt = [0] * 74
    ...
    exBytes = list("example".encode('ascii'))
    for i in range(7):
        dnsPkt[55+i] = exBytes[i]
    dnsPkt[62]=3  # length of "com"
    comBytes = list("com".encode('ascii'))
    for i in range(3):
        dnsPkt[63+i] = comBytes[i]
    ...
    for i in range(0, 74, 4):
        dw = 0
        for j in range(4):
            if (i + j) < 74:
                dw |= (dnsPkt[i+j] & 0xFF) << (j * 8)
        if i < 128:
            buf_emit(0xC7); buf_emit(0x47); buf_emit(i & 0xFF); buf_emit32(dw & 0xFFFFFFFF)
    ...
    for p in (jne_dns_skip, jne_dns_skip2):
    ...
    b_dn = rel_dn & 0xFFFFFFFF
    buf_set(jle_dns_no + 2, b_dn & 0xFF)
    buf_set(jle_dns_no + 3, (b_dn >> 8) & 0xFF)
    buf_set(jle_dns_no + 4, (b_dn >> 16) & 0xFF)
    buf_set(jle_dns_no + 5, (b_dn >> 24) & 0xFF)
    buf_emit(0x59)  # pop rcx
    emit_hlt()
    buf_emit(0xFF); buf_emit(0xC9)
    buf_emit(0x0F); buf_emit(0x85)
    buf_emit32_signed(dns_poll - buf_len() - 4)
    emit_call_puts_str("DNS timeout\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    ...
    b_dg = rel_dg & 0xFFFFFFFF
    buf_set(jmp_dns_got + 1, b_dg & 0xFF)
    buf_set(jmp_dns_got + 2, (b_dg >> 8) & 0xFF)
    buf_set(jmp_dns_got + 3, (b_dg >> 16) & 0xFF)
    buf_set(jmp_dns_got + 4, (b_dg >> 24) & 0xFF)
    ...
    jnz_vesa_ok = buf_len()
    buf_emit(0x0F); buf_emit(0x85); buf_emit32(0)
    emit_call_puts_str("No VESA framebuffer\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    vesa_cmd_ok = buf_len()
    rel_vco = vesa_cmd_ok - jnz_vesa_ok - 6
    b_vco = rel_vco & 0xFFFFFFFF
    buf_set(jnz_vesa_ok + 2, b_vco & 0xFF)
    buf_set(jnz_vesa_ok + 3, (b_vco >> 8) & 0xFF)
    buf_set(jnz_vesa_ok + 4, (b_vco >> 16) & 0xFF)
    buf_set(jnz_vesa_ok + 5, (b_vco >> 24) & 0xFF)
    ...
    # Draw a white banner bar: y=280..320, x=200..824 (624x40 pixels)
    ...
    emit_call_puts_str("VESA: 1024x768x32 drawn\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    emit_call_puts_str("Ring3: entering user mode...\r\n")

    # Save kernel return address at [0x6108] for syscall exit
    ring3_return = buf_len()  # patch the LEA after the IRETQ block
    ...
    b_lr = lea_rel & 0xFFFFFFFF
    buf_set(ring3_return + 3, b_lr & 0xFF)
    buf_set(ring3_return + 4, (b_lr >> 8) & 0xFF)
    buf_set(ring3_return + 5, (b_lr >> 16) & 0xFF)
    buf_set(ring3_return + 6, (b_lr >> 24) & 0xFF)

    emit_call_puts_str("\r\nRing3: user mode test PASSED\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()
    ...
    emit_call_puts_str("Sector 0 hex dump:\r\n")
    ...
    buf_set(jne_no_extra, (after_extra_space - jne_no_extra - 1) & 0xFF)
    ...
    jne_no55aa2 = buf_len()
    emit_call_puts_str("0x55AA (valid)\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    no55aa_off = buf_len()
    buf_set(jne_no55aa, (no55aa_off - jne_no55aa - 1) & 0xFF)
    buf_set(jne_no55aa2, (no55aa_off - jne_no55aa2 - 1) & 0xFF)
    emit_call_puts_str("not found\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)
    patch_cmd_skip()

    # --- Unknown command ---
    emit_call_puts_str("Unknown command. Type 'help'.\r\n")
    buf_emit(0xE9); buf_emit32_signed(cmd_return - buf_len() - 4)

    # =============================================================
    # String Data Section (deferred emission, patched into code above)
    # =============================================================
    for si in range(len(str_data)):
        str_kernel_addr = 0x100000 + buf_len()
        # Emit string bytes + null terminator
        for ch in str_data[si]:
            buf_emit(ord(ch) & 0xFF)
        buf_emit(0)  # null
        # Patch all references to this string
        for patch in str_patches:
            if patch['idx'] == si:
                addr_bytes = str_kernel_addr & 0xFFFFFFFF
                buf_set(patch['loc'], addr_bytes & 0xFF)
                buf_set(patch['loc'] + 1, (addr_bytes >> 8) & 0xFF)
                buf_set(patch['loc'] + 2, (addr_bytes >> 16) & 0xFF)
                buf_set(patch['loc'] + 3, (addr_bytes >> 24) & 0xFF)
    ...
    isr_addr_bytes = isr_data_addr & 0xFFFFFFFFFFFFFFFF
    for p in range(8):
        buf_set(isr_copy_rsi_patch + p, (isr_addr_bytes >> (p * 8)) & 0xFF)

    # Patch the IDT copy source address: 0x100000 + idt_data_offset
    idt_data_addr = 0x100000 + idt_data_offset
    idt_addr_bytes = idt_data_addr & 0xFFFFFFFFFFFFFFFF
    for p in range(8):
        buf_set(idt_copy_rsi_patch + p, (idt_addr_bytes >> (p * 8)) & 0xFF)

    kern_bytes = list(buf)
    kern_sectors = (len(kern_bytes) + 511) // 512
    print("  Kernel: %d bytes (%d sectors)" % (len(kern_bytes), kern_sectors))

    # === Export handwritten-kernel symbol table for hl-compile-pipeline linker ===
    kern_sym = {
        "base": "0x100000",
        "serial_puts": "0x%X" % (0x100000 + serial_puts_off),
        "serial_hex_byte": "0x%X" % (0x100000 + serial_hex_byte_off),
    }
    try:
        kern_sym_path = os.path.join(repo_root, 'bare-kernel', 'kernel-symbols.json')
        with open(kern_sym_path, 'w', encoding='ascii') as f:
            json.dump(kern_sym, f)
        print("  Symbol export: serial_puts=%s" % kern_sym['serial_puts'])
    except Exception as e:
        print("  Symbol export failed: %s" % e)
    ...
    if os.path.exists(kernel_bin_path):
        kb_data = list(bytearray(open(kernel_bin_path, 'rb').read()))
        pad_target = 0x20000  # 128 KB
        if len(kern_bytes) < pad_target:
            padded = [0] * pad_target
            padded[:len(kern_bytes)] = kern_bytes
            kern_bytes = padded
        # Append kernel.bin
        kern_bytes = kern_bytes + kb_data
        kern_sectors = (len(kern_bytes) + 511) // 512
        print("  kernel.bin: %d bytes appended at 0x120000" % len(kb_data))
        print("  _start offset: %s (absolute: 0x%X)" % (kb_entry_offset, 0x120000 + kb_entry_offset))
        print("  Combined kernel: %d bytes (%d sectors)" % (len(kern_bytes), kern_sectors))
    else:
        print("  kernel.bin not found, skipping kernel.bin integration")

    return kern_bytes, kern_sectors

# =========================================
# Stage 2 (real -> protected -> long mode)
# =========================================
def build_stage2(kern_bytes, kern_sectors):
    global buf
    buf = []
    s2_origin = 0x8000
    ...
    buf_set(jne_vbe_fail_patch + 2, rel_fail & 0xFF)
    buf_set(jne_vbe_fail_patch + 3, (rel_fail >> 8) & 0xFF)
    buf_set(jne_vbe_fail_patch + 4, (rel_fail >> 16) & 0xFF)
    buf_set(jne_vbe_fail_patch + 5, (rel_fail >> 24) & 0xFF)
    rel_done = vbe_done_off - (jmp_vbe_done_patch + 3)
    buf_set(jmp_vbe_done_patch + 1, rel_done & 0xFF)
    buf_set(jmp_vbe_done_patch + 2, (rel_done >> 8) & 0xFF)

    # === No real-mode kernel load here ===
    # The kernel is read directly via ATA PIO in 64-bit mode after paging is set up.
    # This avoids the 1MB addressing limit of real-mode ES:BX segment buffers, which
    # could not reach 0x100000 (segment 0x1000 maps to 0x10000 = 64KB, not 1MB).
    ...
    buf_set(j32_patch, addr32 & 0xFF)
    buf_set(j32_patch + 1, (addr32 >> 8) & 0xFF)
    buf_set(j32_patch + 2, (addr32 >> 16) & 0xFF)
    buf_set(j32_patch + 3, (addr32 >> 24) & 0xFF)
    ...
    emit_mov_ecx_imm32(0xC0000080)
    emit_mov_edx_imm32(0)
    emit_mov_eax_imm32(0x101)
    emit_wrmsr()
    # Enable paging
    ...
    buf_set(j64_patch, addr64 & 0xFF)
    buf_set(j64_patch + 1, (addr64 >> 8) & 0xFF)
    buf_set(j64_patch + 2, (addr64 >> 16) & 0xFF)
    buf_set(j64_patch + 3, (addr64 >> 24) & 0xFF)

    # 64-bit: read kernel from disk via ATA PIO directly to 0x100000
    emit_mov_rsp_imm64(0x70000)
    emit_cld()

    # mov rsi, <kernel LBA placeholder>   (patched below after s2_sectors known)
    buf_emit(0x48); buf_emit(0xBE)  # REX.W MOV RSI, imm64
    ata_lba_patch = buf_len()
    buf_emit32(0); buf_emit32(0)
    # mov rdi, 0x100000
    buf_emit(0x48); buf_emit(0xBF)  # REX.W MOV RDI, imm64
    buf_emit32(0x100000); buf_emit32(0)
    # mov rcx, <kernel sector count placeholder>
    buf_emit(0x48); buf_emit(0xB9)  # REX.W MOV RCX, imm64
    ata_sectors_patch = buf_len()
    buf_emit32(0); buf_emit32(0)

    # .sector_loop:
    sector_loop_off = buf_len()
    buf_emit(0x51)  # push rcx
    # drive select: mov edx, 0x1F6; mov eax, esi; shr eax, 24; or al, 0xE0; out dx, al
    emit_mov_edx_imm32(0x1F6)
    buf_emit(0x89); buf_emit(0xF0)                    # mov eax, esi
    buf_emit(0xC1); buf_emit(0xE8); buf_emit(0x18)    # shr eax, 24
    buf_emit(0x0C); buf_emit(0xE0)                    # or al, 0xE0
    emit_out_dx_al()
    # sector count = 1: mov edx, 0x1F2; mov al, 1; out dx, al
    emit_mov_edx_imm32(0x1F2)
    emit_mov_al_imm8(1)
    emit_out_dx_al()
    # LBA low: mov edx, 0x1F3; mov eax, esi; out dx, al
    emit_mov_edx_imm32(0x1F3)
    buf_emit(0x89); buf_emit(0xF0)
    emit_out_dx_al()
    # LBA mid: mov edx, 0x1F4; mov eax, esi; shr eax, 8; out dx, al
    emit_mov_edx_imm32(0x1F4)
    buf_emit(0x89); buf_emit(0xF0)
    buf_emit(0xC1); buf_emit(0xE8); buf_emit(0x08)
    emit_out_dx_al()
    # LBA high: mov edx, 0x1F5; mov eax, esi; shr eax, 16; out dx, al
    emit_mov_edx_imm32(0x1F5)
    buf_emit(0x89); buf_emit(0xF0)
    buf_emit(0xC1); buf_emit(0xE8); buf_emit(0x10)
    emit_out_dx_al()
    # command: mov edx, 0x1F7; mov al, 0x20 (read sectors); out dx, al
    emit_mov_edx_imm32(0x1F7)
    emit_mov_al_imm8(0x20)
    emit_out_dx_al()
    # wait: in al, dx (status); test BSY(0x80) -> retry; test DRQ(0x08) -> retry
    wait_off = buf_len()
    emit_in_al_dx()
    buf_emit(0xA8); buf_emit(0x80)     # test al, 0x80 (BSY)
    buf_emit(0x75)                     # jnz rel8
    bsy_rel_patch = buf_len()
    buf_emit(0)
    buf_emit(0xA8); buf_emit(0x08)     # test al, 0x08 (DRQ)
    buf_emit(0x74)                     # jz rel8
    drq_rel_patch = buf_len()
    buf_emit(0)
    # read 256 words from data port: mov edx, 0x1F0; mov ecx, 256; rep insw
    emit_mov_edx_imm32(0x1F0)
    emit_mov_ecx_imm32(256)
    buf_emit(0xF3); buf_emit(0x66); buf_emit(0x6D)   # rep insw (advances rdi by 512)
    buf_emit(0x48); buf_emit(0xFF); buf_emit(0xC6)   # inc rsi
    buf_emit(0x59)                                   # pop rcx
    buf_emit(0x48); buf_emit(0xFF); buf_emit(0xC9)   # dec rcx
    loop_rel = sector_loop_off - (buf_len() + 2)
    buf_emit(0x75); buf_emit(loop_rel & 0xFF)        # jnz sector_loop

    # Patch wait loop branch displacements (both branch back to wait_off)
    bsy_rel = wait_off - (bsy_rel_patch + 1)
    drq_rel = wait_off - (drq_rel_patch + 1)
    buf_set(bsy_rel_patch, bsy_rel & 0xFF)
    buf_set(drq_rel_patch, drq_rel & 0xFF)
    ...
    buf_set(lgdt_patch, gdtr_addr & 0xFF)
    buf_set(lgdt_patch + 1, (gdtr_addr >> 8) & 0xFF)

    s2_bytes = list(buf)
    s2_sectors = (len(s2_bytes) + 511) // 512
    print("  Stage2: %d bytes (%d sectors)" % (len(s2_bytes), s2_sectors))

    # Patch ATA read parameters in Stage 2 code (placeholders were emitted before s2_sectors was known)
    # Stage1 loads from LBA 1 to 0x8000, so Stage2 starts at 0x8000 and the kernel
    # begins immediately after the Stage2 sectors.
    # The kernel is read by the 64-bit ATA PIO loop directly to 0x100000, which
    # avoids the SeaBIOS ROM/BIOS area (0xF0000-0xFFFFF) and the 1MB addressing
    # limit of real-mode ES:BX buffers.
    s2_kernel_lba = 1 + s2_sectors
    for p in range(8):
        s2_bytes[ata_lba_patch + p] = (s2_kernel_lba >> (p * 8)) & 0xFF
    for p in range(8):
        s2_bytes[ata_sectors_patch + p] = (kern_sectors >> (p * 8)) & 0xFF
    print("  Patched kernel LBA = 0x%X, sectors = %d for ATA read" % (s2_kernel_lba, kern_sectors))

    return s2_bytes, s2_sectors

# =========================================
# Stage 1 (MBR)
# =========================================
def build_stage1(s2_sectors):
    global buf
    ...
    emit_stage1_load_stage2_chs(s2_sectors & 0xFF, 0x8000)
    ...
    s1_bytes = list(buf)
    print("  Stage1: %d bytes (MBR, reads %d sectors)" % (len(s1_bytes), read_sectors))
    return s1_bytes, read_sectors

# =========================================
# Main entry point
# =========================================
def main():
    global buf, repo_root, kernel_bin_path, kernel_entry_path, kb_entry_offset

    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    # Early read of kernel.bin entry offset (needed during kernel codegen at CALL _start)
    kernel_bin_path = os.path.join(repo_root, 'bare-kernel', 'kernel.bin')
    kernel_entry_path = os.path.join(repo_root, 'bare-kernel', 'kernel.entry')
    kb_entry_offset = -1
    if os.path.exists(kernel_bin_path) and os.path.exists(kernel_entry_path):
        with open(kernel_entry_path, 'r') as f:
            kb_entry_offset = int(f.read().strip())
        print("  Early read: kernel.entry offset = %d" % kb_entry_offset)

    print("=== Rebuilding hicos-hl.img ===")

    # Build Kernel
    buf = []
    com1, isr_bytes, idt_data, isr_base, idt_base = build_kernel_part1()
    kern_bytes, kern_sectors = build_kernel_part2(com1, isr_bytes, idt_data, isr_base, idt_base)

    # Build Stage 2
    s2_bytes, s2_sectors = build_stage2(kern_bytes, kern_sectors)

    # Build Stage 1
    s1_bytes, read_sectors = build_stage1(s2_sectors)
    ...
    # Copy Stage 1
    img[0:len(s1_bytes)] = bytearray(s1_bytes)
    # Copy Stage 2
    img[512:512 + len(s2_bytes)] = bytearray(s2_bytes)
    # Copy Kernel
    kern_start = (1 + s2_sectors) * 512
    img[kern_start:kern_start + len(kern_bytes)] = bytearray(kern_bytes)

    # Write image
    out_path = os.path.join(repo_root, 'hicos-hl.img')
    with open(out_path, 'wb') as f:
        f.write(img)
    print("")
    print("  Total: %d bytes (%d sectors)" % (img_size, total_sectors))
    print("  Stage1: sector 0 (MBR, reads %d sectors to 0x8000)" % read_sectors)
    print("  Stage2: sectors 1-%d (real->long mode)" % s2_sectors)
    print("  Kernel: sectors %d-%d (loaded at 0x100000)" % (s2_sectors + 1, total_sectors - 1))
